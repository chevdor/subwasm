...
pub fn get_result(prefix: Prefix, buffer: &[u8]) -> Result<SrhResult> {
	buffer.using_encoded(|wasm_blob| {
		...
		Ok(SrhResult { hash, encoded_hash })
	})
}
...
pub fn get_parachainsystem_authorize_upgrade(wasm_blob: &[u8]) -> Result<CalllHash> {
	...
	let authorize_upgrade_prefix = *decoded2
		.first()
		.ok_or_else(|| RuntimePropHashError::MissingEnvironmentVariable("AUTHORIZE_UPGRADE_PREFIX_ENV"))?;
...
fn get_call_hash(prefix: Prefix, wasm_blob: &[u8]) -> Result<CalllHash> {
	let mut hasher = Blake2bVar::new(SIZE).map_err(|_| RuntimePropHashError::HashComputing())?;
	hasher.update(&concatenate_arrays(&[prefix.0, prefix.1], wasm_blob));
	let mut result: CalllHash = [0; SIZE];
	hasher.finalize_variable(&mut result).map_err(|_| RuntimePropHashError::HashComputing())?;
	Ok(result)
}
...
mod prop_hash_tests {
	...
	fn test_get_result() {
		...
		assert!(res.encoded_hash == "9388ba11b3f2a5db3ef9bf237f1c88ffb369d77ffa843fc67570c89c09fa9c0e");
	}
	...
	fn test_long_input() {
		...
		assert!(res.encoded_hash == "9348da94fcffe94318313f8ce237211a7fd6c1531ab21b61606a1f7eeb8b2409");
	}
}
